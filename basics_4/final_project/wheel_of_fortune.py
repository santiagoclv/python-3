...
class WOFHumanPlayer(WOFPlayer):
    def getMove(self, category, obscuredPhrase, guessed):
        input_message =   '''
            ...
            Guess a letter, phrase, or type 'exit' or 'pass':'''.format(name=self.name, prizeMoney=self.prizeMoney, category=category, obscured_phrase=obscuredPhrase, guessed=guessed)
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getMostPossibleLetter(self, guessed):
        letters = []
        for lt in WOFComputerPlayer.SORTED_FREQUENCIES:
            if(not lt in guessed):
                    letters.append(lt)
        return letters[-1]
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if(len(letters) == 0 or self.prizeMoney < VOWEL_COST):
            return 'pass'
        if(self.smartCoinFlip()):
            return self.getMostPossibleLetter(guessed)
        else:
